...
@router.post("/create_relationship/p2p/type={relationship_type}/")
async def create_relationship(relationship_type: str, file: UploadFile = File(...)):
    try:
        ...
        return JSONResponse(content={"message": "Relationships created successfully", "details": stats}, status_code=200)
    except Exception as e:
        logging.error(f"Error in /create_relationship p2p/: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))


@router.post("/create_relationship/p2u/type={relationship_type}/")
async def create_relationship(relationship_type: str, file: UploadFile = File(...)):
    print("here")
    try:
        contents = await file.read()
...
@router.post("/create_relationship/u2u/type={relationship_type}/")
async def create_relationship(relationship_type: str, file: UploadFile = File(...)):
    print("here")
    try:
        contents = await file.read()
...
@router.post("/create_relationship/u2p/type={relationship_type}/")
async def create_relationship(relationship_type: str, file: UploadFile = File(...)):
    try:
        contents = await file.read()
        logging.info(f"Received file: {file.filename} of type: {file.content_type}")
